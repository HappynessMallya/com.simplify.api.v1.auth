...
docker-compose --file ${FILE_DOCKER_COMPOSE} exec php-fpm php bin/console doctrine:database:create --if-not-exists
echo "*** Command executed successfully ***"

if [ "ls migrations/ | wc -l" > 1 ]; then
  echo "*** Run migrations db ***"
  docker-compose --file ${FILE_DOCKER_COMPOSE} exec php-fpm php bin/console doctrine:migration:migrate --no-interaction
  echo "*** Migrations executed in database ***"
else
  echo "*** Do not migrations to execute ***"
fi

## permissions command here? chmod -R 777 ...
